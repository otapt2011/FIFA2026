...
statusBadge: (status, isLive = false) => {
    ...
    <div class="match-card-clickable ${bgClass} rounded-lg shadow-sm p-3 border border-gray-200 hover:shadow-md transition" data-match-id="${m.id}">
      ...
      <div class="flex items-center justify-between gap-2">
        <div class="flex items-center gap-1 flex-1 justify-end min-w-0">
          <span class="font-semibold text-right truncate" title="${escapeHtml(m.homeTeam)}">${escapeHtml(m.homeTeamCode || m.homeTeam)}</span>
        ...
        <div class="flex items-center gap-1 flex-1 min-w-0">
          ...
          <span class="font-semibold truncate" title="${escapeHtml(m.awayTeam)}">${escapeHtml(m.awayTeamCode || m.awayTeam)}</span>
        </div>
      </div>
  ...
  <div class="space-y-6">
    ...
    <div class="text-center">
      <button id="downloadFullExportBtn" class="bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-lg text-lg w-full sm:w-auto">
        <i class="fas fa-database mr-2"></i>Export Full Database (JSON)
      </button>
      <p class="text-xs text-white/50 mt-1">
        Exports all 10 tables (tournaments, stages, cities, stadiums, groups, teams, matches, bracket_rules, match_scores, match_events) in a structured format.
      </p>
    </div>

    <div class="border-t border-white/10 pt-4">
      <h4 class="font-bold text-indigo-400 mb-2">match_scores</h4>
      <div class="relative">
        <pre class="bg-gray-800 text-green-400 p-3 rounded text-sm overflow-x-auto max-h-60 border border-white/10"><code id="matchScoresJson"></code></pre>
        <div class="absolute top-2 right-2 flex gap-1">
          <button id="copyMatchScoresBtn" class="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-xs"><i class="far fa-copy"></i> Copy</button>
          <button id="downloadMatchScoresBtn" class="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-xs"><i class="fas fa-download"></i> JSON</button>
        </div>
      </div>
    </div>

    <div>
      <h4 class="font-bold text-indigo-400 mb-2">match_events</h4>
      <div class="relative">
        <pre class="bg-gray-800 text-green-400 p-3 rounded text-sm overflow-x-auto max-h-60 border border-white/10"><code id="matchEventsJson"></code></pre>
        <div class="absolute top-2 right-2 flex gap-1">
          <button id="copyMatchEventsBtn" class="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-xs"><i class="far fa-copy"></i> Copy</button>
          <button id="downloadMatchEventsBtn" class="bg-gray-700 hover:bg-gray-600 text-white px-2 py-1 rounded text-xs"><i class="fas fa-download"></i> JSON</button>
        </div>
      </div>
    </div>
  </div>
