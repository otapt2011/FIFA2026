...
export const templates = {
  ...
  matchCard: (m, isLive = false) => {
    ...
    const badge = templates.statusBadge(m.matchStatus);
    let timerSpan = '';
    if (isLive) {
  ...
  matchModal: (match, eventsList, matchId) => {
    ...
    let htmlStr = `<div class="space-y-4"><div class="bg-white/5 p-3 rounded-lg text-center border border-white/10"><div class="font-bold text-lg flex items-center justify-center gap-2 flex-wrap text-white/90"><img src="${escapeHtml(match.homeFlag)}" class="w-6 h-5 object-contain" onerror="this.style.display='none'"> ${escapeHtml(match.homeTeam)} <span class="mx-1">${scoreText}</span> ${escapeHtml(match.awayTeam)} <img src="${escapeHtml(match.awayFlag)}" class="w-6 h-5 object-contain" onerror="this.style.display='none'"></div><div class="text-sm text-white/60 mt-1">${formatDate(match.date)} · ${escapeHtml(match.stadium)} ${match.city ? ', '+escapeHtml(match.city) : ''}</div><div class="text-xs text-white/40">${match.stageName} ${match.groupName ? '· '+match.groupName : ''}</div></div>
    ...
    htmlStr += `<div class="mt-2"><label class="block text-xs font-medium text-white/60">Status</label><select id="editStatus" class="w-full bg-gray-800 border border-white/10 rounded p-1 text-white/90"><option value="scheduled" ${match.matchStatus === 'scheduled' ? 'selected' : ''}>Scheduled</option><option value="finished" ${match.matchStatus === 'finished' ? 'selected' : ''}>Finished</option></select></div>
    <button id="saveScoreLocalBtn" data-match-id="${matchId}" class="mt-3 bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1 rounded text-sm w-full">Save Score</button></div>
    <div class="border-t border-white/10 pt-3 mt-2"><h4 class="font-bold text-indigo-400 mb-2">Match Events</h4>`;
    if (eventsList.length) {
      const homeEvents = eventsList.filter(e => e.team === match.homeTeam);
      const awayEvents = eventsList.filter(e => e.team === match.awayTeam);
      const iconMap = { goal: '<i class="fas fa-futbol text-green-400"></i>', penalty_goal: '<i class="fas fa-futbol text-green-400"></i>', yellow_card: '<i class="fas fa-square text-yellow-400"></i>', red_card: '<i class="fas fa-square text-red-400"></i>', own_goal: '<i class="fas fa-futbol text-red-400"></i>' };
      htmlStr += `<div class="border border-white/10 rounded bg-white/5 p-2"><div class="grid grid-cols-2 gap-0"><div class="pr-3 border-r border-white/10"><div class="text-xs font-semibold text-indigo-400 mb-2 text-center">${escapeHtml(match.homeTeam)} (Home)</div>`;
    ...
    htmlStr += `<div class="bg-white/5 border border-white/10 p-3 rounded mt-3"><h5 class="font-semibold text-sm mb-2 text-white/80">Add Event</h5><div class="grid grid-cols-2 gap-2"><select id="eventTeam" class="bg-gray-800 border border-white/10 rounded p-1 text-sm text-white/90"><option value="${escapeHtml(match.homeTeam)}">${escapeHtml(match.homeTeam)}</option><option value="${escapeHtml(match.awayTeam)}">${escapeHtml(match.awayTeam)}</option></select><input type="text" id="eventPlayer" placeholder="Player name" class="bg-gray-800 border border-white/10 rounded p-1 text-sm text-white/90 placeholder-white/40"><select id="eventType" class="bg-gray-800 border border-white/10 rounded p-1 text-sm text-white/90"><option value="goal">⚽ Goal</option><option value="penalty_goal">⚽ Penalty Goal</option><option value="own_goal">⚽ Own Goal</option><option value="yellow_card">🟨 Yellow Card</option><option value="red_card">🟥 Red Card</option></select><div class="flex gap-1"><input type="number" id="eventMinute" placeholder="Minute" class="bg-gray-800 border border-white/10 rounded p-1 text-sm w-20 text-white/90 placeholder-white/40"><input type="number" id="eventExtra" placeholder="+" value="0" class="bg-gray-800 border border-white/10 rounded p-1 text-sm w-16 text-white/90 placeholder-white/40"></div></div><button id="addEventBtn" data-match-id="${matchId}" class="mt-2 bg-green-600 hover:bg-green-700 text-white px-3 py-1 rounded text-sm w-full">Add Event</button></div></div>`;
    return htmlStr;
  },
