...
function buildUI() {
  ...
  appDiv.innerHTML = `
    ...
    <div id="filterBar" class="px-3 py-2 bg-gray-900/80 border-b border-white/10 flex flex-wrap items-center gap-2 hidden">
      <span class="text-xs text-gray-400">Filter:</span>
      <select id="filterGroup" class="flex-1 min-w-[80px] bg-gray-800 border border-white/10 rounded px-2 py-1 text-xs text-white/80">
        <option value="">All Groups</option>
      </select>
      <select id="filterTeam" class="flex-1 min-w-[80px] bg-gray-800 border border-white/10 rounded px-2 py-1 text-xs text-white/80">
        <option value="">All Teams</option>
      </select>
      <select id="filterStage" class="flex-1 min-w-[100px] bg-gray-800 border border-white/10 rounded px-2 py-1 text-xs text-white/80">
  ...
  state.DOM.filterStage = document.getElementById('filterStage');
...
async function populateFilters() {
  ...
  const stages = await data.getStagesList();
  
  // Build options for groups
  state.DOM.filterGroup.innerHTML = groups.map(g => `<option value="${escapeAttr(g)}">${g}</option>`).join('');
  
  // Build options for teams
  state.DOM.filterTeam.innerHTML = teams.map(t => `<option value="${escapeAttr(t)}">${t}</option>`).join('');
  
  // Build options for stages
  state.DOM.filterStage.innerHTML = stages.map(s => `<option value="${escapeAttr(s)}">${s}</option>`).join('');
}
...
async function login() {
  ...
  const storedToken = localStorage.getItem('fifa_auth_token');
  if (storedToken) {
    return storedToken; // caller will test it; if invalid, it will clear and retry
  }
  ...
  // showLoginModal already stores token on success, but we can ensure it's stored
...
function init() {
  ...
  if (state.DOM.filterStage) {
    state.DOM.filterStage.onchange = () => {
      state.filter.stage = state.DOM.filterStage.value || null;
      if (['fixtures', 'upcoming', 'live'].includes(state.currentTab)) render.switchTab(state.currentTab);
    };
  }
  if (state.DOM.clearFilterBtn) {
    state.DOM.clearFilterBtn.onclick = () => {
      ...
      if (state.DOM.filterStage) state.DOM.filterStage.value = '';
      state.filter.group = null;
      state.filter.team = null;
      state.filter.stage = null;
      if (['fixtures', 'upcoming', 'live'].includes(state.currentTab)) render.switchTab(state.currentTab);
    };
  }
